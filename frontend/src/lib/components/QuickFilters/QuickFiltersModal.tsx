...
export function QuickFiltersModal({ context }: QuickFiltersLogicProps): JSX.Element {
    const { isModalOpen, view, modalTitle, modalWidth } = useValues(quickFiltersModalLogic({ context }))
    const { closeModal } = useActions(quickFiltersModalLogic({ context }))

    return (
        <LemonModal title={modalTitle} isOpen={isModalOpen} onClose={closeModal} width={modalWidth}>
