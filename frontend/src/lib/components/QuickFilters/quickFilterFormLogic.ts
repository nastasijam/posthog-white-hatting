    ...
    reducers(({ props }) => ({
        ...
        options: [
            (props.filter?.options || [
                { value: null, label: '', operator: PropertyOperator.Exact },
            ]) as QuickFilterOption[],
            {
                addOption: (state) => [...state, { value: null, label: '', operator: PropertyOperator.Exact }],
        ...
        isValid: [
            ...
            (name, propertyName, options): boolean => {
                ...
                return options.every((opt: QuickFilterOption) => {
                    ...
                    if (operatorNeedsValue && (opt.value === null || !opt.value.trim())) {
                        return false
                    }
        ...
        submitForm: async () => {
            ...
            const payload = {
                ...
                options: values.options.map((opt: QuickFilterOption) => ({
                    value: opt.value !== null ? opt.value.trim() : null,
