...
from posthog.schema import QuickFilterContext

from posthog.api.routing import TeamAndOrgViewSetMixin
from posthog.models.quick_filter import QuickFilter, QuickFilterContextMembership
...
class QuickFilterSerializer(serializers.ModelSerializer):
    ...
    def validate_options(self, value):
        ...
        for option in value:
            if not isinstance(option, dict):
                raise ValidationError("Each option must be an object with 'value', 'label', and 'operator' fields")
            if "value" not in option or "label" not in option or "operator" not in option:
                raise ValidationError("Each option must have 'value', 'label', and 'operator' fields")

            if option["value"] is not None and not isinstance(option["value"], str):
                raise ValidationError("Option 'value' must be a string or null")
    ...
    def validate_contexts(self, value):
        ...
        valid_contexts = [c.value for c in QuickFilterContext]
    ...
    def create(self, validated_data):
        ...
        for context in contexts:
            QuickFilterContextMembership.objects.create(
